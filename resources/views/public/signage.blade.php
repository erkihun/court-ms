...
<body class="min-h-screen text-gray-900">
    ...
    <div
        class="fixed top-4 right-4 z-20 flex items-center gap-2 rounded-full bg-white px-4 py-2 shadow-lg ring-1 ring-slate-900/5">
    ...
    <div class="max-w-6xl mx-auto min-h-screen p-4 sm:p-6 lg:p-8">
        <div class="grid gap-6 xl:grid-cols-2 items-start">
            ...
            <div class="flex flex-col gap-6">
                @php
                $displayNow = $now;
                $sanitizeAnnouncementContent = function (?string $content): string {
                    // Keep announcement HTML within the configured default profile.
                    return \Mews\Purifier\Facades\Purifier::clean($content ?? '', 'default');
                };
                @endphp
                    ...
                    <div class="relative px-6 py-6 md:px-8 md:py-8">
                        ...
                        <div class="relative z-10 flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
                            ...
                            <div class="flex items-center gap-5">
                                <div
                                    class="h-20 w-20 rounded-2xl border-2 border-white/20 bg-white/10 p-3 shadow-xl backdrop-blur-sm flex items-center justify-center">
                                    @if($settings?->logo_path)
                                    <img class="h-full w-full object-contain drop-shadow-lg"
                                        src="{{ \Illuminate\Support\Facades\Storage::url($settings->logo_path) }}"
                                        alt="logo">
                                    @else
                                    <img class="h-full w-full object-contain drop-shadow-lg"
                                        src="{{ asset('favicon.ico') }}" alt="logo">
                                    @endif
                                </div>
                                <div>
                    ...
                    <div class="rounded-2xl border border-slate-200 bg-white shadow-soft overflow-hidden">
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h2 class="text-lg font-bold text-slate-900">
                                            {{ __('signage.case_distribution') }}</h2>
                                        <p class="text-sm text-slate-500 mt-0.5">{{ __('signage.status_breakdown') }}
                                        </p>
                                    </div>
                                </div>
                                <span
                                    class="inline-flex items-center gap-1 rounded-full border border-orange-200 bg-orange-50 px-3 py-1 text-xs font-semibold text-orange-700">
                        ...
                        <div class="p-6">
                            <div class="grid gap-4">
                                @php
                                $statusLabelMap = [
                                'pending' => __('signage.status_labels.pending'),
                                'active' => __('signage.status_labels.active'),
                                'closed' => __('signage.status_labels.closed'),
                                'hearing' => __('signage.status_labels.hearing'),
                                'dismissed' => __('signage.status_labels.dismissed'),
                                ];
                                @endphp
                                ...
                                'hearing' => ['bg' => 'bg-blue-600', 'text' => 'text-blue-700', 'border' =>
                                'border-blue-200', 'light' => 'bg-blue-50'],
                                'dismissed' => ['bg' => 'bg-rose-600', 'text' => 'text-rose-700', 'border' =>
                                'border-rose-200', 'light' => 'bg-rose-50'],
                                ];
                                $statusKey = strtolower($stat->label);
                                $color = $colors[$statusKey] ?? $colors['active'];
                                $statusLabel = $statusLabelMap[$statusKey] ?? $stat->label;
                                @endphp
                                <div
                                    class="flex items-center gap-4 p-3 rounded-xl border {{ $color['border'] }} bg-white hover:bg-slate-50 transition-colors">
                                    <div class="flex-1">
                                        <div class="flex items-center justify-between mb-2">
                                            <span class="font-semibold text-slate-900">{{ $statusLabel }}</span>
                                            <span
                                                class="text-lg font-bold text-slate-900">{{ number_format($stat->total) }}</span>
                                        </div>
                                        <div class="flex items-center gap-3">
                                            <div class="flex-1 h-2.5 rounded-full bg-slate-200 overflow-hidden">
                                                <div class="h-full rounded-full {{ $color['bg'] }} transition-all duration-700"
                                                    style="width: {{ $pct }}%;"></div>
                                            </div>
                                            <span
                                                class="text-sm font-bold {{ $color['text'] }} min-w-[3rem] text-right">{{ $pct }}%</span>
                                        </div>
                                    </div>
                                </div>
                                @endforeach
                        ...
                        <div class="border-b border-slate-100 bg-gradient-to-r from-slate-50 to-white px-6 py-5">
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h2 class="text-lg font-bold text-slate-900">{{ __('signage.case_categories') }}
                                        </h2>
                                    ...
                                    <div class="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <div class="mt-2 flex items-center gap-2">
                                                ...
                                                <span
                                                    class="text-sm text-slate-500">{{ round(($cat->total / $maxCategory) * 100) }}%</span>
                                            </div>
                                        </div>
                                    </div>
                                    <span class="text-sm font-medium text-slate-500">{{ __('signage.cases') }}</span>
                                </div>
                                @endforeach
                            </div>
                        </div>
                    ...
                    <div class="border-b border-slate-100 bg-gradient-to-r from-slate-50 to-white px-6 py-5">
                        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <h2 class="text-lg font-bold text-slate-900">{{ __('signage.todays_activity') }}
                                    </h2>
                        ...
                        <div class="grid gap-6 lg:grid-cols-2">
                                    ...
                                    <table class="w-full text-sm table-sticky">
                                        <thead class="bg-slate-50">
                                            <tr class="border-b border-slate-200">
                                                <th
                                                    class="py-3 px-4 text-left text-xs font-semibold uppercase tracking-wide text-slate-600">
                                                    {{ __('signage.case_number') }}</th>
                                                <th
                                                    class="py-3 px-4 text-left text-xs font-semibold uppercase tracking-wide text-slate-600">
                                                    {{ __('signage.applicant_column') }}</th>
                                                <th
                                                    class="py-3 px-4 text-left text-xs font-semibold uppercase tracking-wide text-slate-600">
                                                    {{ __('cases.case_type') }}</th>
                                            </tr>
                                        </thead>
                                        <tbody class="divide-y divide-slate-100">
                                            ...
                                            <tr class="hover:bg-slate-50 transition-colors">
                                                @php
                                                $unknownApplicantLabel = __('signage.unknown_applicant');
                                                $applicantFull = trim((string) ($case->applicant?->full_name ?? ''));
                                                if ($applicantFull === '') {
                                                $applicantFull = $case->applicant_name ?? $case->applicant?->name ??
                                                $unknownApplicantLabel;
                                                }
                                                @endphp
                                                <td class="py-3 px-4">
                                                    <span
                                                        class="font-bold text-slate-800 font-mono text-sm">{{ $case->case_number ?? __('signage.case_number').' '.$case->id }}</span>
                                                </td>
                            ...
                            <div class="space-y-4">
                                <div class="flex items-center justify-between">
                                    <div>
                                        <h3 class="text-base font-bold text-slate-900">
                                            {{ __('signage.scheduled_hearings') }}</h3>
                                        <p class="text-sm text-slate-500 mt-0.5">{{ __('signage.live_status') }}</p>
                                    </div>
                                    <span
                                        class="text-xs font-semibold text-amber-600 bg-amber-50 px-2.5 py-1 rounded-full">{{ __('signage.today') }}</span>
                                </div>
                                <div
                                    class="overflow-hidden rounded-xl border border-slate-200 max-h-[300px] overflow-y-auto scrollbar-thin">
                                    <table class="w-full text-sm table-sticky">
                ...
                <div class="mt-6 space-y-3">
                    <div
                        class="rounded-2xl border border-slate-200 bg-gradient-to-r from-white to-slate-50 p-4 shadow-soft">
                        <div class="flex items-center justify-between mb-3">
                            <div>
                                <h3 class="text-base font-bold text-slate-900">
                                    {{ __('signage.active_announcements') }}</h3>
                                <p class="text-xs text-slate-500">{{ __('signage.today') }}</p>
                            </div>
                            <span
                                class="text-xs font-semibold text-emerald-600 bg-emerald-50 px-3 py-1 rounded-full border border-emerald-100">
                                {{ $activeAnnouncements->count() }} {{ __('signage.active_announcements') }}
                            </span>
                        </div>
                        <div class="space-y-4">
                            @forelse($activeAnnouncements as $announcement)
                            <article class="rounded-xl border border-slate-100 bg-white/80 p-3">
                            ...
                            @empty
                            <p class="text-sm text-slate-500">{{ __('signage.no_active_announcements') }}
                            </p>
                            @endforelse
                        </div>
                    </div>
                ...
                <div class="rounded-2xl border border-slate-200 bg-white shadow-soft overflow-hidden">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-slate-500 mt-0.5">
                                        {{ __('signage.currently_active_personnel') }}
                                    </p>
                                </div>
                            </div>
                            <span
                                ...
                                {{ $activeStaff->count() }} {{ __('signage.active_staff_label') }}
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="p-6">
                        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                            ...
                            <div
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <p class="text-sm text-slate-600 truncate">
                                        {{ $person->position ?? __('signage.staff_position_fallback') }}</p>
                                    <div class="mt-2 flex flex-wrap items-center gap-2 text-xs">
                                        <span
                                            class="inline-flex items-center gap-1.5 rounded-md border border-emerald-200 bg-emerald-50 px-2.5 py-1 font-semibold text-emerald-700">
                            ...
                            @empty
                            <div
                                class="rounded-xl border-2 border-dashed border-slate-200 bg-slate-50 p-10 text-center">
                        ...
                        <div class="mt-8 pt-6 border-t border-slate-200">
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-3">
                                    <div
                                        class="flex h-10 w-10 items-center justify-center rounded-lg bg-gradient-to-br from-slate-100 to-slate-50">
