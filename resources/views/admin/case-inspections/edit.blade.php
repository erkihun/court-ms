<x-admin-layout title="{{ __('case_inspections.edit_heading') }}">
    @section('page_header', __('case_inspections.edit_heading'))

    <div class="p-6 bg-white rounded-xl border border-gray-200">
        <h1 class="text-2xl font-semibold text-gray-900 mb-4">{{ __('case_inspections.edit_heading') }}</h1>
        <form method="POST" action="{{ route('case-inspections.update', $caseInspection) }}" class="space-y-6">
            @method('PATCH')
            @include('admin.case-inspections._form')
            ...
            <div class="flex items-center gap-3">
                ...
                <button type="submit" class="px-4 py-2 rounded-md bg-blue-600 text-white text-sm hover:bg-blue-700">
                    {{ __('case_inspections.form_update') }}
                </button>
            </div>
        </form>
    </div>
