<x-admin-layout title="{{ __('case_inspections.create_heading') }}">
    @section('page_header', __('case_inspections.create_heading'))

    <div class="p-6 bg-white rounded-xl border border-gray-200">
        <h1 class="text-2xl font-semibold text-gray-900 mb-4">{{ __('case_inspections.create_heading') }}</h1>
        <form method="POST" action="{{ route('case-inspections.store') }}" class="space-y-6">
            @include('admin.case-inspections._form', ['caseInspection' => null])

            <div class="flex items-center gap-3">
                <a href="{{ route('case-inspections.index') }}" class="px-4 py-2 rounded-md border border-gray-300 text-gray-700 text-sm">
    ...
    @push('scripts')
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                tinymce.init({
                    selector: '#case-inspection-details',
                    license_key: 'gpl',
                    height: 420,
