<x-admin-layout title="Bench Notes">
    @section('page_header', 'Bench notes')
    ...
    <div class="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
            <h1 class="text-2xl font-semibold text-gray-900">Bench notes</h1>
            <p class="text-sm text-gray-600">Internal notes tied to cases (sanitized via HTML Purifier).</p>
        </div>
    ...
    <div class="mb-4 rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
        <form method="GET" class="grid gap-3 md:grid-cols-[2fr_auto_auto] md:items-end">
            <div>
                <label class="block text-sm font-medium text-gray-700">Filter by case</label>
                <select name="case_id"
                ...
                <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                    <div class="flex-1">
                        ...
                        <p class="text-sm text-gray-600 bg-blue-50 px-3 py-2 rounded-lg border border-blue-100">
                            <span class="font-medium text-blue-700">Case:</span> {!! $safeCaseTitle !!}
                        </p>
                        @endif
                    </div>
                ...
                <div class="meta-grid">
                    <div class="meta-item">
                        ...
                        <div class="meta-content">
                            ...
                            <div class="meta-value">{{ $note->user?->name ?? '—' }}</div>
                        </div>
                    </div>
    ...
    <div class="empty-state">
        ...
        <h3 class="text-xl font-semibold text-gray-900 mb-2">No bench notes found</h3>
        <p class="text-gray-600 mb-6">
            @if($caseId)
            No notes found for this case. Create your first note to get started.
            @else
            Get started by creating your first bench note.
            @endif
        </p>
        <a href="{{ route('bench-notes.create', ['case_id' => $caseId]) }}"
