<x-admin-layout title="{{ __('cases.case_details') }}">
    ...
    <div class="p-4 mb-4 rounded-xl bg-white border border-gray-200 shadow-sm">
        <form method="GET" class="flex gap-3 items-end">
            <div class="flex-grow">
                <label for="q-search" class="block text-xs font-medium text-gray-700 mb-1">{{ __('cases.search') }}</label>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
            ...
            <div>
                <label for="filter-status"
                    class="block text-xs font-medium text-gray-700 mb-1">{{ __('cases.table.status') }}</label>
            ...
            <div class="md:col-span-2 flex gap-3">
                <div>
                    <label for="filter-from"
                        class="block text-xs font-medium text-gray-700 mb-1">{{ __('cases.filters.from') }}</label>
                    <input id="filter-from" type="date" name="from" value="{{ optional($from)->format('Y-m-d') }}"
                        class="w-full px-3 py-2 rounded-lg bg-gray-50 text-gray-900 border border-gray-300 focus:ring-blue-500 focus:border-blue-500 shadow-inner">
                </div>
                <div>
                    <label for="filter-to"
                        class="block text-xs font-medium text-gray-700 mb-1">{{ __('cases.filters.to') }}</label>
        ...
        <table class="min-w-full divide-y divide-gray-200 text-sm">
            ...
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ __('cases.show.review_actions') }}
                    </th>
                    <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex flex-wrap gap-2">
                            <a href="{{ route('cases.show', $c->id) }}"
                                class="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg border border-blue-200 bg-blue-50 text-blue-700 text-xs font-semibold shadow-sm hover:bg-blue-100 hover:border-blue-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition">
                                {{ __('cases.table.view') }}
                            </a>
                ...
                <tr>
                    ...
                    <td colspan="9" class="px-6 py-12 text-center text-gray-500 text-base">
                        ...
                        <h3 class="mt-2 text-sm font-medium text-gray-900">{{ __('cases.table.no_cases_found') }}</h3>
