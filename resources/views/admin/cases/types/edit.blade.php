<x-admin-layout title="{{ __('Edit Case Type') }}">
    ...
    <div class="p-6 rounded-xl border border-gray-200 bg-white shadow-sm">
        <form method="POST" action="{{ route('case-types.update', $type->id) }}" class="space-y-4">
            @csrf @method('PATCH')
            <div>
            ...
            <div class="flex gap-2">
                ...
                <button class="px-4 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white">{{ __('Update') }}</button>
            </div>
        </form>
    </div>
</x-admin-layout>