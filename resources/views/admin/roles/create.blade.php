<x-admin-layout title="{{ __('roles.create.title') }}">
    @section('page_header', __('roles.create.title'))

    <div class="max-w-6xl mx-auto space-y-6">
        <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-6 flex items-start justify-between gap-4">
            <div>
                <h1 class="text-xl font-semibold text-gray-900">{{ __('roles.create.title') }}</h1>
                <p class="text-sm text-gray-600 mt-1">{{ __('roles.create.subtitle', ['count' => $perms->count()]) }}</p>
            </div>
            <a href="{{ route('roles.index') }}"
                ...
                {{ __('roles.create.back') }}
            </a>
        </div>

        <form method="POST" action="{{ route('roles.store') }}" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            @csrf
            ...
            <div class="lg:col-span-1 space-y-4">
                <div class="p-5 rounded-xl border border-gray-200 bg-white shadow-sm space-y-3">
                    <div>
                        <label class="block text-sm font-medium text-gray-700">{{ __('roles.fields.name_unique') }}</label>
                        ...
                        <p class="text-xs text-gray-500 mt-1">{{ __('roles.create.name_hint') }}</p>
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700">{{ __('roles.fields.description') }}</label>
                        <textarea name="description" rows="3"
                            class="mt-1 w-full px-3 py-2 rounded-lg bg-white text-gray-900 border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200">{{ old('description') }}</textarea>
                        @error('description') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                        <p class="text-xs text-gray-500 mt-1">{{ __('roles.create.description_hint') }}</p>
                    </div>
                </div>

                <div class="p-5 rounded-xl border border-gray-200 bg-white shadow-sm space-y-3">
                    <h3 class="text-sm font-semibold text-gray-800">{{ __('roles.create.summary_title') }}</h3>
                    <ul class="text-sm text-gray-700 space-y-2">
                        <li class="flex items-center gap-2">
                            <span class="h-2 w-2 rounded-full bg-emerald-500"></span>
                            {{ __('roles.create.summary_permissions', ['count' => $perms->count()]) }}
                        </li>
                        <li class="flex items-center gap-2">
                            <span class="h-2 w-2 rounded-full bg-indigo-500"></span>
                            {{ __('roles.create.summary_roles_total') }}
                        </li>
                    </ul>
                </div>
            </div>
            ...
            <div class="lg:col-span-2 p-5 rounded-xl border border-gray-200 bg-white shadow-sm space-y-4">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xs text-gray-500">{{ __('roles.create.permissions_hint') }}</p>
                    </div>
        ...
        (() => {
            ...
            toggleBtn.addEventListener('click', () => {
                ...
                toggleBtn.textContent = shouldCheck
                    ? @json(__('roles.create.deselect_all'))
                    : @json(__('roles.create.select_all'));
            });
        })();
    </script>
    @endpush
